...
export async function main() {
    ...
    const csv = await fs.readFile(join(process.cwd(), 'scripts', 'bookmarksCSV2YAML', 'table.csv'), 'utf8')
    const lines = csv.split('\n')
    console.log(lines.length)
    lines.forEach(line => {
        const [title, tag, description, url, status, image, openSource, updated, published] = line.split(';')
